...
namespace FlatUI
{
    public class FlatAlertBox : Control
    {
        ...
        public void ShowControl(_Kind _Kind, string Str, int Interval)
        {
            Kind = _Kind;
            Text = Str;
            Visible = true;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                case _Kind.Success:                   
                    _with14.FillRectangle(new SolidBrush(SuccessColor), Base); //-- Base
                    ...
                    //-- Checked Sign
                    _with14.DrawString("ü", new Font("Wingdings", 22), new SolidBrush(SuccessText), new Rectangle(7, 7, W, H), Helpers.Main.NearSF);
                    _with14.DrawString(Text, Font, new SolidBrush(SuccessText), new Rectangle(48, 12, W, H), Helpers.Main.NearSF);

                    //-- X button
                    _with14.FillEllipse(new SolidBrush(Color.FromArgb(35, Color.Black)), new Rectangle(W - 30, H - 29, 17, 17));
                    _with14.DrawString("r", new Font("Marlett", 8), new SolidBrush(SuccessColor), new Rectangle(W - 28, 16, W, H), Helpers.Main.NearSF);

                    if(State == Helpers.MouseState.Over)
                        _with14.DrawString("r", new Font("Marlett", 8), 
                            new SolidBrush(Color.FromArgb(25, Color.White)), 
                            new Rectangle(W - 28, 16, W, H), Helpers.Main.NearSF);
                    break;
                case _Kind.Error:
                    _with14.FillRectangle(new SolidBrush(ErrorColor), Base); //-- Base
                    ...
                    //-- X Sign
                    _with14.DrawString("r", new Font("Marlett", 16), new SolidBrush(ErrorText), new Rectangle(6, 11, W, H), Helpers.Main.NearSF);
                    _with14.DrawString(Text, Font, new SolidBrush(ErrorText), new Rectangle(48, 12, W, H), Helpers.Main.NearSF);

                    //-- X button
                    _with14.FillEllipse(new SolidBrush(Color.FromArgb(35, Color.Black)), new Rectangle(W - 32, H - 29, 17, 17));
                    _with14.DrawString("r", new Font("Marlett", 8), new SolidBrush(ErrorColor), new Rectangle(W - 30, 17, W, H), Helpers.Main.NearSF);

                    if(State == Helpers.MouseState.Over)
                        _with14.DrawString("r", new Font("Marlett", 8), 
                            new SolidBrush(Color.FromArgb(25, Color.White)), 
                            new Rectangle(W - 30, 15, W, H), Helpers.Main.NearSF);
                    break;
                case _Kind.Info:
                    _with14.FillRectangle(new SolidBrush(InfoColor), Base); //-- Base
                    ...
                    //-- Info Sign
                    _with14.DrawString("¡", new Font("Segoe UI", 20, FontStyle.Bold), new SolidBrush(InfoText), new Rectangle(12, -4, W, H), Helpers.Main.NearSF);
                    _with14.DrawString(Text, Font, new SolidBrush(InfoText), new Rectangle(48, 12, W, H), Helpers.Main.NearSF);

                    //-- X button
                    _with14.FillEllipse(new SolidBrush(Color.FromArgb(35, Color.Black)), new Rectangle(W - 32, H - 29, 17, 17));
                    _with14.DrawString("r", new Font("Marlett", 8), new SolidBrush(InfoColor), new Rectangle(W - 30, 17, W, H), Helpers.Main.NearSF);

                    if(State == Helpers.MouseState.Over)
                        _with14.DrawString("r", new Font("Marlett", 8), 
                            new SolidBrush(Color.FromArgb(25, Color.White)), 
                            new Rectangle(W - 30, 17, W, H), Helpers.Main.NearSF);
                    break;
            }
